...
# Env vars that .env may inject and that leak across tests via StorageConfig.
...
def _clean_dotenv_leaks():
    ...
    Some test files call create_app() without deps, which triggers load_dotenv().
    This leaks GAIA_LANCEDB_URI (s3://...) into the process env, causing later
    tests to connect to remote S3 instead of local tmp_path.
    """
    yield
