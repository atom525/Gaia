...
Graph IR 由四种实体构成：**Proposition**（命题）、**Strategy**（推理声明）、**Operator**（结构约束）、**FormalExpr**（策略展开）。
...
表示推理算子，连接 Proposition。统一 `type` 字段：
...
确定性逻辑关系（equivalence, contradiction, complement, implication, disjunction, conjunction）。对应 theory Layer 3 的势函数，所有算子均确定性（ψ ∈ {0, 1}，无自由参数）。Schema 见 [graph-ir.md](graph-ir.md) §3。

### FormalExpr（策略展开）

Strategy 在 Operator 层的展开——记录一个 Strategy 由哪些 Operator 和中间 Proposition 构成。9 种命名策略自带 FormalExpr，确定性策略可自动生成，非确定性策略需 reviewer 手动创建。Schema 见 [graph-ir.md](graph-ir.md) §4。
...
规范化（lcn → gcn 映射）见 [graph-ir.md](graph-ir.md) §5。
...
- **node_priors**：只有 `type=claim` 的 Proposition 有记录。
...
- 组装结果必须覆盖所有 claim Proposition 和所有 Strategy，否则 BP 拒绝运行。
...
- **beliefs**：只有 `type=claim` 的 Proposition 有 belief。
...
| 对象 | 内容 | 变化频率 |
|------|------|---------|
| **LocalCanonicalGraph** | 包内 Proposition + Strategy（含 steps）+ 完整文本 | 每次 build 更新 |
| **GlobalCanonicalGraph** | 跨包 Proposition（引用 lcn）+ 全局 Strategy（无 steps）+ Operator + FormalExpr | 每次 ingest/curation 更新 |
...
| **FormalExpr** | Strategy 在 Operator 层的展开（含中间 Proposition） | 分类确认时创建 |
| **BeliefState** | 全局 claim 的后验信念 + resolution policy | 每次 global BP 创建 |

## 源代码

- `libs/graph_ir/models.py` -- `LocalCanonicalGraph`, `Proposition`, `Strategy`
- `libs/storage/models.py` -- global `Proposition`, `CanonicalBinding`, `BeliefSnapshot`
